...
-export([deep_merge/2, deep_value_merge/2]).
...
deep_merge(M1, M2) when is_map(M1), is_map(M2) ->
    deep_merge(M1, M2, fun deep_merge/2);
deep_merge(_, Override) ->
    Override.

deep_merge(M1, M2, GoDeep) when is_map(M1), is_map(M2) ->
    maps:fold(
        fun(K, V2, Acc) ->
                V1 = maps:get(K, Acc, undefined),
                NewV = deep_merge(V1, V2, GoDeep),
                Acc#{K => NewV}
        end, M1, M2);
deep_merge(V1, V2, GoDeep) ->
    GoDeep(V1, V2).

deep_value_merge(#{?HOCON_T := array, ?HOCON_V := V1} = Base,
                 #{?HOCON_T := object, ?HOCON_V := V2} = Top) ->
    NewV = do_deep_value_merge(V1, V2),
...
deep_value_merge(V1, V2) ->
    do_deep_value_merge(V1, V2).

do_deep_value_merge(M1, M2) when is_map(M1) andalso is_map(M2) ->
    deep_merge(M1, M2, fun deep_value_merge/2);
do_deep_value_merge(V1, V2) ->
...
do_merge_array(Array, [{I, Value} | Rest]) ->
    GoDeep = fun(Elem) -> deep_value_merge(Elem, Value) end,
